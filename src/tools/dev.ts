...
- **Adding a field/relationship to an existing resource** (core's \`UserResource\`/\`DiscussionResource\`/\`PostResource\`, or another extension's): use \`Extend\\ApiResource(TargetResource::class)->fields(MyFields::class)\`, where \`MyFields\` is an invokable, container-resolved class returning \`Schema\\*\` objects (\`Schema\\Boolean::make('canX')\`, \`Schema\\Str\`, \`Schema\\Relationship\\ToOne\`/\`ToMany\`). Read with \`->get(fn ($model, $context) => …)\`; make writable with \`->writable(fn ($model, $context) => $context->getActor()->can(...))\` plus \`->set(...)\` (permission gate on \`writable\`, the mutation in \`set\`); hide output with \`->visible(false)\` or a closure. **Fail closed**: a per-row \`->get()\`/\`->visible()\` that reads \`can()\`/state must try/catch to a safe default, because a throw there 500s the whole collection (every row of an index, or the \`forum\` object on every page). Inject services into the fields-class constructor, never \`resolve()\`/\`app()\` inside the closure. Avoid N+1: eager-load via the relationship \`->scope()\` / endpoint default-includes, and use \`Schema\\Integer::make(...)->countRelation('x')\` for counts, not a per-row \`count()\`. Adding to \`ForumResource\` (loads on every page, has no backing model so every field needs \`->get()\`): keep getters O(1) with no per-request queries. To target another extension's resource, wrap the \`ApiResource\` in \`Extend\\Conditional()->whenExtensionEnabled('<ext-id>', ...)\`.
- **Adding a notification type**: register \`Extend\\Notification()->type(MyBlueprint::class, ['alert', 'email'])\` (the second arg is the channels pre-checked in the user's preference grid; 2.0 has no serializer arg). The Blueprint implements \`BlueprintInterface\` (\`getFromUser\`/\`getSubject\`/\`getData\` + static \`getType\`/\`getSubjectModel\`), plus the marker \`AlertableInterface\` for the alert channel and \`MailableInterface\` (with an \`Extend\\View\` namespace + blade templates) for email; a channel's preference only exists if its interface is implemented. **Send** by injecting \`Flarum\\Notification\\NotificationSyncer\` and calling \`->sync(new MyBlueprint(...), $recipients)\`: it honors each user's per-channel preference for you (don't check it yourself), and re-syncing with a smaller recipient list retracts the alert. Frontend: register the alert component with \`Extend.Notification().add('myType', MyComponent)\` and add a \`NotificationGrid\` \`notificationTypes\` row (keys must equal the \`getType()\` string). Gotchas: never notify the actor (\`getFromUser\`); resolve recipients inside a queued job and chunk the query; store any link data you need in \`getData()\` since the subject may be deleted before send; a notification whose subject isn't a \`Discussion\` (and has no \`discussion()\` relation) groups under the forum title unless you override \`NotificationList\` content.
...
- **Request-scoped state, never statics or singletons.** Under a persistent runtime (FrankenPHP/Swoole/Octane/RoadRunner) one process serves many requests and users, so request state left in a \`static\` or an app-scoped singleton bleeds across them (a privilege-leak class of bug, not just staleness). Resolve the actor fresh per request via \`Flarum\\Http\\RequestUtil::getActor($request)\` (never stash the \`User\`); read input from the PSR-7 \`ServerRequestInterface\` (\`getQueryParams\`/\`getParsedBody\`/\`getCookieParams\`/\`getHeaderLine\`), never \`$_GET\`/\`$_POST\`/\`$_COOKIE\`/\`$_SERVER\`; don't depend on \`register_shutdown_function\`/\`exit\`/request-end side effects or on memory being reclaimed (don't accumulate per-request data into a singleton); keep singletons stateless (no cached request/actor/settings snapshot). This is the same discipline a queue or multi-server handler needs. Set \`Secure\` cookies only on HTTPS.
...
- **Assets vs runtime files.** Ship static JS/CSS via \`Extend\\Frontend()->css()/->js()\` (there is no \`Extend\\Asset\` for bundles); \`flarum assets:publish\` copies them onto the assets disk, so they follow the admin's S3 driver automatically. Files generated at runtime (uploads, exports, generated images) go to a disk **you** declared, never into the published-assets tree. An uploaded file or a \`tmpfile()\` is valid only within the one request on the one server that holds it, so stream it onto a disk before the request ends and never hand a queued job a local temp path. Set disk **visibility** (\`Visibility::PUBLIC\`/\`PRIVATE\`), don't \`chmod\`.
- **Portable URLs (sub-path, multi-domain, reverse proxy, CDN).** Everything derives from the \`url\` in \`config.php\` (Flarum never sniffs the request host), so generate URLs through it, never by hand. Backend: inject \`Flarum\\Http\\UrlGenerator\` and use \`->to('forum'|'api'|'admin')->route('name', $params)\`/\`->path(...)\`/\`->base()\`; register routes with \`Extend\\Routes\`. Frontend: use \`app.route(name, params)\` and \`app.forum.attribute('baseUrl'|'basePath'|'apiUrl'|'assetsBaseUrl')\`, never \`window.location\` or a hardcoded \`/\`. Resolve any static file through the \`flarum-assets\` disk's \`->url()\`: the assets host may be a CDN/S3 on a different origin, so never hardcode \`/assets/...\`. Behind a TLS-terminating proxy, correctness comes from setting \`url\` to the external \`https://...\` value; core has no \`trustedProxies\`/\`X-Forwarded\` handling, so don't read \`$_SERVER\` to infer scheme or host. Emails and any other out-of-request URL must be absolute, built via \`UrlGenerator\`.`,
  },
  integrations: {
...
- **flarum/tags** (\`flarum-tags\`): present on most forums, so be tag-aware. Read a discussion's tags via the \`tags\` relation (\`$discussion->tags\`, model \`Flarum\\Tags\\Tag\`) only when enabled. **Tag-scoped permissions** are ordinary group-permission rows prefixed \`tag{id}.\`, and only \`is_restricted\` tags gate anything. To make your own ability tag-scopable, register it as a discussion permission (so it appears in the per-tag grid) and check it on the **Discussion** with \`$actor->can('myext.doThing', $discussion)\`: \`DiscussionPolicy\` then applies the per-tag gate automatically. For bulk filtering use \`Tag::whereHasPermission($actor, 'ability')\` (it already encodes restricted-vs-global and parent inheritance), not a manual loop, and never write \`tag{id}.\` rows yourself (tags maintains them). Frontend: \`discussion.tags()\`, gated by \`'flarum-tags' in flarum.extensions\` + \`flarum.reg.get('flarum-tags', ...)\`. Gotchas: a child tag inherits its parent's restriction; a discussion is hidden if the actor lacks the permission in **any** of its restricted tags; and the primary-tag-count settings can block \`startDiscussion\`, so creating a discussion programmatically must attach a valid primary tag (or the actor must hold \`bypassTagCounts\`).
