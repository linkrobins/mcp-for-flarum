...
- **The Docker image is now built for `linux/arm64` as well as `linux/amd64`.** Apple Silicon is a large share of the machines these clients run on, and until now those users were pulling an amd64 image and running it under emulation, with a platform-mismatch warning.
- **The published image is public.** It had been private since an organization change, so `docker pull ghcr.io/linkrobins/mcp-for-flarum` failed for everyone except its owner — which meant the only install path the README documented was, in practice, broken for the public.
- **Documented what was already shipping.** The `flarum_troubleshoot` tool, its `FLARUM_TROUBLESHOOT` switch, and all four prompts (`build-flarum-extension`, `review-flarum-extension`, `check-flarum-compatibility`, `prepare-flarum-support-request`) are on by default but appeared nowhere in the README, so there was no way to discover them short of reading the source. They now have their own sections.
- The README documents both install paths as equals, notes that the project is not on the npm registry (so a package appearing under that name is not this project), and drops the pitch for the Link Robins managed tier, which no longer exists. The `DIAG_URL` / `SNAPSHOT_URL` hooks are now described for what they are — optional, dormant unless pointed at a backend, and MIT like the rest — rather than as a paid tier self-hosters cannot reach.
- The license section states plainly that commercial and hosted use are welcome and need no permission: no separate license, no key, no tier that unlocks anything.
