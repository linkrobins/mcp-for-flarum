...
- **The published image is public.** It had been private since an organisation change, so `docker pull ghcr.io/linkrobins/mcp-for-flarum` failed for everyone except its owner — which meant the only install path the README documented was, in practice, broken for the public.
...
- The licence section states plainly that commercial and hosted use are welcome and need no permission: no separate licence, no key, no tier that unlocks anything.
